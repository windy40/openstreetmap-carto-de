...
def index_statement(table, name, function, conditions=None, concurrent=False,notexist=False, fillfactor=None):
    ...
    return ('CREATE INDEX{options} {table}_{name}\n' +
            ...
            '{storage}' +
            '{where};\n').format(table="planet_osm_hstore_"+table, name=name, function=function,
                storage=storage, options=options, where=where)

def parse(cb):
    with open(os.path.join(os.path.dirname(__file__), '../indexes-hstore.yml')) as yaml_file:
...
def reindex_cb(table, name, function, where):
    if not args.concurrent:
        print('REINDEX planet_osm_hstore_{table}_{name};'.format(table=table, name=name))
    else:
        # Rebuilding indexes concurrently requires making a new index, dropping the old one, and renaming.
        print('ALTER INDEX planet_osm_hstore_{table}_{name} RENAME TO planet_osm_hstore_{table}_{name}_old;'.format(table=table, name=name))
        cb(table, name, function, where)
        print('DROP INDEX planet_osm_hstore_{table}_{name}_old;\n'.format(table=table, name=name))
